...
async def code_evaluation_with_gemini(prompt_str: str, content: list, language: str) -> dict:
    logger.info(f"Evaluating code for language review_code_with_gemini: {language}")
    ...
    try:
        ...
        if not code_analyser_agent:
            raise ValueError("code_evaluation_agent not initialized")
        else:
            logger.info("code_evaluation_agent initialized")

            response = await code_analyser_agent.run(prompt)
            logger.info(f"Code Evaluation generated")
            ...
            try:
                ...
                logger.error(f"code_code_evaluation_agent_agent did not return valid JSON.\nOutput was:\n{text_output}")
                raise ValueError(f"code_code_evaluation_agent_agent did not return valid JSON.\nOutput was:\n{text_output}")
            return parsed

    except google_exceptions.GoogleAPICallError as e:
        logger.error(f"Gemini API error while evaluating code: {str(e)}")
        raise ValueError(f"Gemini API error while evaluating code: {str(e)}")
    except Exception as e:
        logger.error(f"Unexpected error evaluating code: {str(e)}")
        raise ValueError(f"Unexpected error evaluating code: {str(e)}")
...
def get_or_extract_code(request):
    try:
        ...
        logger.error(f"Unexpected error in fetching code: {str(e)}")